...
const VideoPage = () => {
  ...
  const getSimilarVideo = async()=>{
    const idData = await fetch(SIMILAR_VIDEO_API+id)
        ...
        const VideoId = (idJson.items.map((item => {
          ...
          <div className='flex justify-between flex-col md:flex-row mt-4 '>
            ...
            <div className='flex text-white mt-4 md:mt-0'>
                <div className='flex items-center justify-center h-11 px-6 rounded-3xl bg-white/[0.15]'>
                  ...
                  <span>{`${abbreviateNumber(video[0]?.statistics?.likeCount, 2)} likes`} </span>
                </div>
                <div className='flex items-center justify-center h-11 px-6 rounded-3xl bg-white/[0.15] ml-4'>
                  <span>{`${abbreviateNumber(video[0]?.statistics?.viewCount, 2)} views`} </span>
                </div>
              </div>
          </div>}
          <div className='text-white font-medium text-sm md:text-lg h-16 mt-4 line-clamp-2'>{!video?null:(video[0]?.snippet?.description)}</div>
       
          
        </div>
        <div className='flex flex-col py-6 px-4 overflow-y-auto lg:w-[350px] xl:w-[400px]'>
        {!similarVideo ?null :similarVideo.map((data)=>{
            return(
              <SimilarVideoCard key={data?.id} video={data.snippet} statistics={data.statistics} videoId={data?.id}/>
              )
          }) }
        </div>
      </div>
